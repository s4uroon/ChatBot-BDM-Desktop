...
class SettingsDialog(QDialog):
    ...
    def _validate_api_settings(self, api_key: str, base_url: str, model: str) -> Tuple[bool, str]:
        ...
        if len(api_key.strip()) < 10:
            return False, "La clé API semble trop courte (minimum 10 caractères)."
        ...
        try:
            ...
            if not parsed.netloc:
                return False, "L'URL est invalide (domaine manquant)."
        except Exception as e:
        ...
        if len(model.strip()) < 2:
            return False, "Le nom du modèle semble trop court."
    ...
    def _on_save_clicked(self):
        ...
        # Validation
        valid, error_msg = self._validate_api_settings(api_key, base_url, model)
        if not valid:
            QMessageBox.warning(
                self,
